...
vgs
```
For my logical volumes I have sized the partitions based on various best practice rules that I have picked up over the years. This is by no meeans a strict rule, but it is advised to split up `/boot`, `/var`, and `/home`. The `/boot` directory has been placed on a seperate partition due to the encryption we're going to use. We will also create some [swap space](http://linuxjournal.com/article/10678){:target="_blank"}. In this exampe I am using a 64GB disk
- /boot = 512MB
- /var = 10GB
- swap = 8GB
- / = 20GB
...
cat /mnt/etc/fstab
```
To configure the rest of the system we're going to use [chroot](https://en.wikipedia.org/wiki/Chroot){:target="_blank"} to change the root directory. This makes it easier to configure
```
arch-chroot /mnt
```
...
```
Next we need to install and configure the [GRUB](https://en.wikipedia.org/wiki/GNU_GRUB){:target="_blank"} bootloader
```
pacman -S grub
y
